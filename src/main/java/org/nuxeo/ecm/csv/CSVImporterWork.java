	...
	protected boolean createDocument(long lineNumber, String parentPath,
		...
		try {
			...
			if (session.exists(parentRef)) {
				...
				//Proto specific code to add new missing parent of type Subject
				if(type.equals(CSV_SUBJECT_TYPE)){
					...
					//Finally creates the current Subject
					createDocument(lineNumber, parentPath, name, CSV_SUBJECT_TYPE, properties);}
				else 
				{
